...
import xyz.sadcenter.lightning.bot.sources.utils.RegisterUtil;
...
public final class TakeCommand extends Command {
    ...
    public void handle(Member user, Message message, TextChannel textChannel, String... args) {
        ...
        if(Boostrap.getInstance().getInGameNamesManager().exists(name)) {
        ...
        if (Boostrap.getInstance().getInGameNamesManager().getName(user.getIdLong()) != null) {
        ...
        user.getGuild().addRoleToMember(user.getIdLong(), Objects.requireNonNull(user.getGuild().getRoleById(Boostrap.getInstance().getConfiguration().registeredId))).queue();
        Boostrap.getInstance().getInGameNamesManager().register(user.getIdLong(), name);
